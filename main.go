...
var re = regexp.MustCompile(`(?i)CREATE TYPE (\w+) AS ENUM\s*\(([\w\d-><=_',\s]+)\);`)
...
func TransformEnums(items []string) []string {
	for index := range items {
		var buf bytes.Buffer
		for _, char := range items[index] {
			if char == '\'' {
				continue
			}
			buf.WriteRune(char)
		}
		items[index] = strings.TrimSpace(buf.String())
	}
	return items
}
...
func main() {
	...
	for {
		...
		if len(cleanBuffer) >= 11 {
			if cleanBuffer[:11] == "CREATE TYPE" {
				...
				values := TransformEnums(strings.Split(matches[2], ","))
	...
	// Unescape html to allow for > < signs.
	outBuffer.WriteString(html.UnescapeString(str))
...
const (
	{{range $val := $values -}}
       {{$typeName}}{{toCamelCase $val}} {{$typeName}} = "{{$val}}"
	{{end -}}
)
